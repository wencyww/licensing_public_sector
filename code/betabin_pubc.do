...
* Load data
	...
	use regression_with_lemp.dta, clear
...
* Totals for licensed
	...
	* Licensed
	bys state_occ: egen sum_pubc = sum(public)
	bys state_occ: egen obs_pubc = count(public)
...
* For-loops
	...
	* Licensed
	...
	file open TABLES using "betabin_results_pubc.txt", write replace
	file write TABLES "occ, a_pubc, b_pubc" _n
	
	forvalues i = 1/`max' {

			// di `i' " of " `max'
			
			quietly summ sh_pubc if occ_n == `i' [aw=obs_pubc]
			local test = r(mean)
			if `test' != 0 {
	...
	quietly copy "betabin_results_pubc.txt" "betabin_results_pubc.csv", replace
	file close TABLES
	...
	* Step 1: Create leave out mean cell shares for licensed, other certification, and unionization
	...
	clear
	use regression_with_lemp

	
		* Licensed share
		egen n_obs_pubc = count(public), by(state_occ)
		bys state_occ: egen ct_pubc = sum(public) if n_obs_pubc > 1
		gen lo_mean_pubc = ct_pubc - public if n_obs_pubc > 1
		replace lo_mean_pubc = lo_mean_pubc/(n_obs_pubc-1) if n_obs_pubc > 1
	...
	* Step 2: Use beta-binomial results to adjust licensed shares
		...
		* Load beta-binomial results
		...
		import delimited "/Users/wenchenwang/Desktop/Paper_Updated_data/4. shares/betabin_results_pubc.csv", encoding(ISO-8859-1) clear
		save tmp2
			
		restore
		merge m:1 occ using tmp2, nogen
		...
		* Adjust values
		...
		rename sh_pubc public_sh
		
		save regression_with_all_sh
